...
- **Implementación (piloto):** validación de intervalo y detección de solapes entre estancias persistidas como `YYYY-MM-DD` viven en `packages/shared-types/src/hotel-calendar.ts` (paquete `@piloto/shared-types`). El API reexporta desde `apps/api/src/domain/shared/hotel-calendar.ts` y la UI del piloto importa el mismo módulo, usando siempre el mismo `timezone_iana` que `hotel_settings` (comprobación de IANA vía `Intl`; comparaciones de fechas en calendario civil del hotel, no instantes del cliente).
